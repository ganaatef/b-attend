...
export default async function SchedulesPage({ searchParams }: { searchParams: Promise<{ date?: string }> }) {
  ...
  if (isBranchManager) scheduleWhere.branchId = { in: managedBranchIds };
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-4">
      <div className="flex items-center justify-between">
        ...
        {!isBranchManager && <Link href="/schedules/bulk" className="inline-flex items-center gap-1.5 rounded-md bg-primary px-3 py-1.5 text-xs font-semibold text-primary-foreground hover:bg-primary/90"><Plus className="h-3.5 w-3.5" /> {t("bulkGenerate")}</Link>}
      </div>
