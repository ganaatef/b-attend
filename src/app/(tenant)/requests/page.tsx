...
export default async function RequestsPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-4">
      ...
      {employee ? (
        <Card>
          <CardHeader className="pb-3"><CardTitle className="text-sm font-semibold text-foreground">{t("newRequest")}</CardTitle></CardHeader>
          <CardContent><RequestForm employeeId={employee.id} branchId={employee.branchId ?? undefined} /></CardContent>
        </Card>
      ) : (
        <Card><CardContent className="py-6"><p className="text-center text-sm text-muted-foreground">{t("noLinkedEmployee")}</p></CardContent></Card>
      )}

      <Card>
        <CardHeader className="pb-3"><CardTitle className="text-sm font-semibold text-foreground">{t("recentRequests")}</CardTitle></CardHeader>
        <CardContent>
          {requests.length === 0 ? <EmptyState title={t("noRequests")} /> : (
            <div className="space-y-2">
              {requests.map((r) => (
                <div key={r.id} className="rounded-md border border-border bg-card px-3 py-2 text-sm">
                  <div className="flex items-center justify-between">
                    <span className="font-medium text-foreground">{displayApprovalType(r.type, locale)}</span>
                  ...
                  {r.date && <p className="text-xs text-muted-foreground">{t("for")} <bdi dir="ltr">{new Date(r.date).toLocaleDateString(locale === "ar" ? "ar-EG" : "en-US")}</bdi></p>}
                  {r.managerNotes && <p className="mt-1 text-xs text-foreground">{t("manager")} {r.managerNotes}</p>}
                </div>
              ))}
            </div>
          )}
