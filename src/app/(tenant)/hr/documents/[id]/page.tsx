      ...
      {isExpiring && (
        <Card className="border-amber-300 bg-amber-50/40">
          <CardContent className="pt-4 pb-4">
            <p className="text-xs font-medium text-amber-700">This document expires on {doc.expiryDate ? new Date(doc.expiryDate).toLocaleDateString() : "—"} — renew soon.</p>
          </CardContent>
        </Card>
      )}
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Card className="border-border p-4">
          ...
          <p className="text-sm font-semibold text-foreground">
            ...
            {isExpiring && <span className="ml-2 text-xs text-amber-600 font-medium">Expiring soon</span>}
          </p>
        </Card>
      </div>
              ...
              <>
                <form action={async () => {
                  ...
                  <button type="submit" className="inline-flex items-center gap-1.5 rounded-md bg-brand-success px-3 py-1.5 text-xs font-medium text-white hover:bg-brand-success/90">
                    <CheckCircle2 className="h-3.5 w-3.5" /> Approve (Valid)
                  </button>
                </form>
                <form action={async () => {
                  "use server";
            ...
            {doc.status !== "MISSING" && (
              <form action={async () => {
                ...
                <button type="submit" className="inline-flex items-center gap-1.5 rounded-md border border-border px-3 py-1.5 text-xs font-medium text-foreground hover:bg-muted/40">
                  <HelpCircle className="h-3.5 w-3.5" /> Mark Missing
                </button>
              </form>
            )}
          </CardContent>
