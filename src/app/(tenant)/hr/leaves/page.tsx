    ...
    <div className="mx-auto max-w-6xl space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{t("pendingRequests", { count: pendingCount })} · {t("onLeaveToday", { count: onLeaveToday })}</p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-4">
        ...
        <Card className="border-border p-4">
          ...
          <p className="text-xs text-muted-foreground">{t("approvedAllTime")}</p>
        </Card>
        <Card className="border-border p-4">
          <p className="text-2xl font-bold text-foreground">{onLeaveToday}</p>
          <p className="text-xs text-muted-foreground">{t("onLeaveTodayCount")}</p>
        </Card>
        <Card className="border-border p-4">
          <p className="text-2xl font-bold text-foreground">{leaveTypes.filter((lt) => lt.active).length}</p>
          <p className="text-xs text-muted-foreground">{t("activeLeaveTypes")}</p>
        </Card>
      </div>
      ...
      <Tabs defaultValue={tab || "requests"} className="space-y-4">
        <TabsList>
          ...
          <TabsTrigger value="balances">{t("balancesTab")}</TabsTrigger>
          {isOwnerOrHr && <TabsTrigger value="types">{t("typesTab")}</TabsTrigger>}
        </TabsList>
        ...
        <TabsContent value="requests">
          <Card className="border-border">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <CardTitle className="text-sm font-semibold text-foreground">{t("leaveRequests")}</CardTitle>
            ...
            <CardContent>
              {leaveRequests.length === 0 ? (
                <EmptyState title={t("noLeaves")} icon={CalendarDays} />
              ) : (
        ...
        <TabsContent value="balances">
          <Card className="border-border">
            <CardHeader className="pb-3"><CardTitle className="text-sm font-semibold text-foreground">{t("leaveBalances", { year: new Date().getFullYear() })}</CardTitle></CardHeader>
            <CardContent>
                  ...
                  {leaveBalances.map((lb) => (
                    <div key={lb.id} className="flex items-center justify-between px-4 py-3">
                      ...
                      <div className="text-right text-xs">
                        <p className="font-medium text-foreground">{t("remaining", { remaining: lb.remaining, total: lb.openingBalance + lb.accrued })}</p>
                        <p className="text-muted-foreground">{t("used")}: {lb.used} · {t("pending")}: {lb.pending}</p>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {isOwnerOrHr && (
          <TabsContent value="types">
            <Card className="border-border">
              <CardHeader className="pb-3">
                <CardTitle className="text-sm font-semibold text-foreground">{t("leaveTypesTitle")}</CardTitle>
              </CardHeader>
              <CardContent>
                {canManageTypes && (
                  <Card className="border-border p-4 mb-4">
                    <h3 className="mb-3 text-xs font-semibold text-foreground">{t("addLeaveType")}</h3>
                    <form action={async (formData: FormData) => { "use server"; await createLeaveTypeAction({}, formData); }} className="flex flex-wrap gap-2 items-end">
                      <div className="flex-1 min-w-[120px]">
                        <Label htmlFor="lt-name" className="sr-only">{t("employee")}</Label>
                        <Input id="lt-name" name="name" required placeholder={t("namePlaceholder")} className="h-8 text-xs" />
                      </div>
                      <div className="w-24">
                        <Label htmlFor="lt-code" className="sr-only">{t("codePlaceholder")}</Label>
                        <Input id="lt-code" name="code" required placeholder={t("codePlaceholder")} className="h-8 text-xs" />
                      </div>
                      <div className="w-32">
                        <Label htmlFor="lt-days" className="sr-only">{t("daysCount")}</Label>
                        <Input id="lt-days" name="annualAllowanceDays" type="number" min="0" defaultValue="0" className="h-8 text-xs" />
                      </div>
                      <label className="flex items-center gap-1.5 text-xs text-muted-foreground">
                        <input type="checkbox" name="paid" defaultChecked className="h-3.5 w-3.5" /> {t("paid")}
                      </label>
                      <label className="flex items-center gap-1.5 text-xs text-muted-foreground">
                        <input type="checkbox" name="requiresApproval" defaultChecked className="h-3.5 w-3.5" /> {t("requiresApproval")}
                      </label>
                      <Button type="submit" size="sm" className="h-8">{t("add")}</Button>
                    </form>
                  </Card>
                )}
                    ...
                    {leaveTypes.map((lt) => (
                      <div key={lt.id} className="flex items-center justify-between px-4 py-3">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            {t("daysPerYear", { count: lt.annualAllowanceDays })} · {lt.paid ? t("paid") : t("unpaid")} · {lt.requiresApproval ? t("requiresApproval") : t("autoApprove")} · {lt.carryForwardAllowed ? t("carryForward") : t("noCarryForward")}
                          </p>
                        </div>
                        <div className="flex items-center gap-2">
