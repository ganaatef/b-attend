    ...
    <div className="mx-auto max-w-2xl space-y-4">
      <div>
        ...
        <div className="mt-1 flex items-center justify-between">
          ...
          <Badge variant={course.active ? "default" : "outline"} className="text-[10px]">{course.active ? "Active" : "Inactive"}</Badge>
        </div>
      </div>

      {course.description && (
        <Card className="border-border">
          <CardHeader className="pb-2"><CardTitle className="text-sm font-semibold text-foreground">Description</CardTitle></CardHeader>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Card className="border-border p-4">
          <p className="text-xs text-muted-foreground">Required for Job Title</p>
          <p className="text-sm font-semibold text-foreground">{course.requiredForJobTitle ?? "—"}</p>
        </Card>
        <Card className="border-border p-4">
          <p className="text-xs text-muted-foreground">Validity</p>
          <p className="text-sm font-semibold text-foreground">{course.validityMonths ? `${course.validityMonths} months` : "No expiry"}</p>
        </Card>
        <Card className="border-border p-4">
          <p className="text-xs text-muted-foreground">Status</p>
          <p className="text-sm font-semibold text-foreground">{course.active ? "Active" : "Inactive"}</p>
        </Card>
      </div>
