    ...
    <div className="mx-auto max-w-5xl space-y-4">
          ...
          {subscription && plan ? (
            <>
              ...
              <div className="text-2xl font-bold text-foreground">
                {plan.isTrial ? t("freeTrial") : plan.isCustom ? t("customPlan") : money(subscription.billingCycle === "MONTHLY" ? subscription.monthlyAmount : subscription.annualAmount, subscription.currency)}
              ...
              {subscription.trialEndsAt && (
                <div className="rounded-md border border-amber-300 bg-amber-50/40 p-2 text-xs text-amber-800">
                  {t("trialEndsOn", { date: new Date(subscription.trialEndsAt).toLocaleDateString() })}
                </div>
              )}
            </>
      ...
      <Card>
        ...
        <CardContent>
          ...
          {plan?.features && (
            <div className="mt-4">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{t("featuresLabel")}</p>
      ...
      <Card>
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <CardTitle className="text-sm font-semibold text-foreground">{t("invoices")}</CardTitle>
            <Link href="/support" className="text-xs text-brand-accent hover:underline">{t("contactBillingLink")}</Link>
          </div>
        </CardHeader>
        <CardContent>
              ...
              <table className="w-full text-sm">
                <thead className="border-b border-border text-xs text-muted-foreground">
                  <tr>
                    ...
                    <th className="px-3 py-2 text-left font-medium">{t("total")}</th>
                    <th className="hidden px-3 py-2 text-left font-medium sm:table-cell">{t("dueDate")}</th>
      ...
      <Card>
        <CardHeader className="pb-3"><CardTitle className="text-sm font-semibold text-foreground">{t("needToUpgrade")}</CardTitle></CardHeader>
        <CardContent className="space-y-2">
          ...
          <Link href="/support" className="inline-flex items-center gap-1.5 rounded-md bg-primary px-3 py-1.5 text-xs font-semibold text-primary-foreground hover:bg-primary/90">
            <FileBarChart className="h-3.5 w-3.5" /> {t("contactBillingBtn")}
          </Link>
        </CardContent>
      </Card>
    </div>
