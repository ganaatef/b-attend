...
export function TipForm() {
  ...
  return (
    <form action={formAction} className="grid gap-3 sm:grid-cols-2">
      ...
      <div className="sm:col-span-2"><Label htmlFor="body">Body *</Label><Textarea id="body" name="body" rows={3} required placeholder="A short, practical tip for the team..." /></div>
      <div>
      ...
      <div className="sm:col-span-2 flex items-center gap-3">
        ...
        {state.ok && <p className="text-xs text-brand-success">Tip created.</p>}
        <Button type="submit" size="sm" disabled={pending} className="ml-auto">{pending ? <><Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" /> Saving...</> : "Add tip"}</Button>
      </div>
    </form>
  );
}
