...
export default async function TenantDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const usage = [
    { label: "Branches", used: branches.length, limit: plan?.maxBranches ?? 0 },
    { label: "Employees", used: tenant._count.employees, limit: plan?.maxEmployees ?? 0 },
    { label: "Managers", used: tenant._count.users, limit: plan?.maxManagers ?? 0 },
    { label: "Schedules", used: tenant._count.schedules, limit: 0 },
    { label: "Punches", used: tenant._count.punches, limit: 0 },
  ];
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-xs font-medium text-muted-foreground">Business type</CardTitle></CardHeader>
          <CardContent>
            <p className="text-sm font-medium text-foreground">{tenant.businessType.replace(/_/g, " ")}</p>
            <p className="text-xs text-muted-foreground">{tenant.city ?? "No city set"}</p>
            <p className="mt-2 text-xs text-muted-foreground">{tenant.employeesCount} employees · {tenant.branchesCount} branches (declared at signup)</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-xs font-medium text-muted-foreground">Subscription</CardTitle></CardHeader>
          <CardContent>
            {tenant.subscription ? (
              <>
                ...
                {tenant.subscription.trialEndsAt && (
                  <p className="mt-1 text-xs text-amber-700">Trial ends: {new Date(tenant.subscription.trialEndsAt).toLocaleDateString()}</p>
                )}
              </>
        ...
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-xs font-medium text-muted-foreground">Plan limits</CardTitle></CardHeader>
          <CardContent>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-semibold text-foreground">Branches ({branches.length})</CardTitle>
          </CardHeader>
          <CardContent>
            {branches.length === 0 ? <EmptyState title="No branches" icon={Building2} /> : (
              <div className="space-y-2">
                {branches.map((b) => (
                  <div key={b.id} className="flex items-center justify-between rounded-md border border-border bg-card px-3 py-2 text-sm">
                    ...
                    <span className="text-xs text-muted-foreground">{b._count.employees} emp</span>
                  </div>
                ))}
              </div>
            )}
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-semibold text-foreground">Recent employees ({tenant._count.employees} total)</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-semibold text-foreground">Invoices ({tenant._count.invoices})</CardTitle>
          </CardHeader>
          <CardContent>
            {invoices.length === 0 ? <EmptyState title="No invoices yet" icon={FileBarChart} /> : (
              <div className="space-y-2">
                {invoices.map((inv) => (
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-semibold text-foreground">Change plan / billing cycle</CardTitle>
          </CardHeader>
          <CardContent>
            {tenant.subscription ? <ChangePlanForm tenantId={tenant.id} currentPlanId={tenant.subscription.planId} currentCycle={tenant.subscription.billingCycle} /> : <p className="text-sm text-muted-foreground">No subscription to modify.</p>}
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-sm font-semibold text-foreground">Create manual invoice</CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-sm font-semibold text-foreground">Recent audit log</CardTitle>
        </CardHeader>
        <CardContent>
          {auditLogs.length === 0 ? <EmptyState title="No audit events" icon={ScrollText} /> : (
