  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">{t("summary", { platform: platformUsers.length, tenant: tenantUsers.length })}</p>
      </div>

      <Card className="border-border">
        <div className="border-b border-border px-4 py-3 text-sm font-semibold text-foreground">{t("platformUsers")}</div>
        {platformUsers.length === 0 ? <EmptyState title={t("noPlatformUsers")} icon={Users} /> : (
          <table className="w-full text-sm">
            <thead className="border-b border-border text-xs text-muted-foreground">
              <tr>
                ...
                <th className="px-4 py-2.5 text-left font-medium">{t("lastLogin")}</th>
              </tr>
            </thead>
            <tbody>
              {platformUsers.map((u) => (
      ...
      <Card className="border-border">
        <div className="border-b border-border px-4 py-3 text-sm font-semibold text-foreground">{t("tenantUsers")}</div>
        {tenantUsers.length === 0 ? <EmptyState title={t("noTenantUsers")} icon={Users} /> : (
            ...
            <table className="w-full text-sm">
              <thead className="sticky top-0 border-b border-border bg-card text-xs text-muted-foreground">
                <tr>
                  <th className="px-4 py-2.5 text-left font-medium">{t("name")}</th>
                  <th className="px-4 py-2.5 text-left font-medium">{t("email")}</th>
                  <th className="px-4 py-2.5 text-left font-medium">{t("tenant")}</th>
                  <th className="px-4 py-2.5 text-left font-medium">{t("role")}</th>
                  <th className="px-4 py-2.5 text-left font-medium">{t("status")}</th>
                </tr>
              </thead>
