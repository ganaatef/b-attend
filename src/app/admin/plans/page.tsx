  ...
  return (
    <div className="mx-auto max-w-7xl space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">{t("planCount", { count: plans.length })}</p>
      </div>
        ...
        {plans.map((p) => (
          <Card key={p.id} className="border-border p-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-muted-foreground">{p._count.subscriptions} {t("subs")}</span>
            </div>
            <p className="mt-2 text-2xl font-bold text-foreground">
              {p.isTrial ? t("free") : p.isCustom ? t("custom") : `${formatNumber(p.priceMonthly)} EGP${t("perMonth")}`}
            </p>
