...
export async function GET(request: NextRequest) {
  ...
  const [balances, leaveRequests, leaveTypes] = await Promise.all([
    db.leaveBalance.findMany({
      where: { employeeId: context.employee.id, year: currentYear, companyId: context.employee.companyId },
...
export async function POST(request: NextRequest) {
  ...
  if (overlap) return NextResponse.json({ error: "OVERLAPPING_LEAVE_REQUEST" }, { status: 409 });
  ...
  if (balance && balance.remaining - balance.pending < daysCount) {
    return NextResponse.json({ error: "INSUFFICIENT_LEAVE_BALANCE" }, { status: 422 });
  }
  ...
  const leaveRequest = await db.$transaction(async (tx) => {
    ...
    if (leaveType.requiresApproval) {
      ...
      await tx.leaveBalance.updateMany({
        where: { employeeId: context.employee.id, leaveTypeId: leaveType.id, year: startDate.getFullYear(), companyId: context.employee.companyId },
        data: { used: { increment: daysCount }, remaining: { decrement: daysCount } },
      });
    }
