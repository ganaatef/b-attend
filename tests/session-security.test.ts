...
describe("Session security", () => {
  it("session.ts uses HS256 algorithm", async () => {
    ...
    const content = fs.readFileSync(path.resolve(__dirname, "../src/lib/auth/session.ts"), "utf-8");
  ...
  it("password-reset.ts uses SHA-256 for token hashing", async () => {
    const fs = await import("fs");
    const path = await import("path");
    const content = fs.readFileSync(path.resolve(__dirname, "../src/lib/auth/password-reset.ts"), "utf-8");
  ...
  it("proxy.ts has no hardcoded JWT secret", async () => {
    const fs = await import("fs");
    const path = await import("path");
    const content = fs.readFileSync(path.resolve(__dirname, "../src/proxy.ts"), "utf-8");
    expect(content).not.toMatch(/secret123|hardcoded.*secret|my.*secret/i);
  });

  it("change-password route is accessible through the proxy", async () => {
    const fs = await import("fs");
    const path = await import("path");
    const content = fs.readFileSync(path.resolve(__dirname, "../src/proxy.ts"), "utf-8");
