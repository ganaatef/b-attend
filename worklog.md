...
---
Task ID: HR-1 (B-Attend HR Module Phase HR-1 — database models, permissions, feature gates, Excel helper, HR navigation, seed data)
Agent: Super Z (orchestrator, direct execution)
Task: Phase HR-1 of the HR module spec — add all HR Prisma models, enums, permissions, feature gates, ExcelJS exporter, HR sidebar navigation, and seed data.

Work Log:
- Installed `exceljs@4.4.0` for real XLSX file generation.
- Added 19 new Prisma models: JobTitle, EmployeeContract, EmployeeDocument, LeaveType, LeavePolicy, LeaveBalance, LeaveRequest, EmployeeWarning, TrainingCourse, TrainingAssignment, EmployeeSkill, Asset, AssetAssignment, OnboardingTask, OffboardingTask, PayrollProfile, PayrollRun, PayrollRunLine, PayrollAdjustment.
- Added 15 new enums: ContractType, ContractStatus, DocumentType, DocumentStatus, LeaveRequestStatus, WarningType, WarningSeverity, WarningStatus, TrainingCategory, TrainingStatus, SkillLevel, AssetType, AssetStatus, OnboardingTaskStatus, OffboardingTaskStatus, SalaryType, PayrollRunStatus, PayrollRunLineStatus, PayrollAdjustmentType, PayrollAdjustmentStatus, HrFeature.
- Extended Employee model with 14 new HR fields: arabicName, dateOfBirth, gender, maritalStatus, address, emergencyContactName, emergencyContactPhone, photoUrl, jobTitleId, grade, probationEndDate, contractType, workLocation, onboardingStatus, offboardingStatus + 12 new reverse relations (contracts, documents, leaveBalances, leaveRequests, warnings, trainingAssignments, skills, assetAssignments, onboardingTasks, offboardingTasks, payrollProfile, payrollRunLines, payrollAdjustments).
- Extended Department with jobTitles reverse relation.
- Extended Tenant with 15 new HR reverse relations.
- Created `src/lib/hr/feature-gates.ts` — 8 HR feature keys (hr_core, hr_documents, hr_leave, hr_training, hr_assets, hr_payroll, hr_excel_export, excel_export) with plan availability: Trial=hr_core, Starter=+docs+leave, Growth=+training+assets+excel, Pro=+payroll, Enterprise=all.
- Created `src/lib/hr/permissions.ts` — 17 HR permissions (VIEW_HR_DASHBOARD, MANAGE_DEPARTMENTS, MANAGE_JOB_TITLES, VIEW_EMPLOYEE_SENSITIVE_DATA, MANAGE_CONTRACTS, MANAGE_DOCUMENTS, MANAGE_LEAVE_TYPES, MANAGE_LEAVE_BALANCES, APPROVE_LEAVE, MANAGE_WARNINGS, MANAGE_TRAINING, MANAGE_ASSETS, MANAGE_ONBOARDING, MANAGE_OFFBOARDING, VIEW_PAYROLL, MANAGE_PAYROLL, EXPORT_HR_EXCEL) with role-based defaults (Owner=all, HR_Admin=all, Branch_Manager=VIEW_HR_DASHBOARD+APPROVE_LEAVE, Employee=none) + salary visibility check + managed branch IDs helper.
- Created `src/lib/excel/exporter.ts` — real XLSX export using ExcelJS with: createWorkbook(), addReportHeaderSheet() (company name, report title, generated by, timestamp, filters, plan note, privacy note), addWorksheetFromRows() (styled headers, frozen row, auto-filter, alternating row colors), formatHeaderRow() (navy bg, white text, bold), autoFitColumns() (content-based width, clamped 10-50), applyRTLIfNeeded() (Arabic RTL support), sendWorkbookResponse() (proper MIME type + Content-Disposition), formatMinutesAsHours(), safeExcelValue(), excelFilename().
- Updated Sidebar with 14 HR nav items for Owner/HR (HR Dashboard, Departments, Job Titles, Contracts, Documents, Leave Management, Warnings, Training, Assets & Uniforms, Payroll Profiles, Payroll Runs, Onboarding, Offboarding, HR Reports) + 6 new icons (Briefcase, FileText, GraduationCap, Package, Wallet, UserPlus, UserMinus, AlertTriangle, FolderTree, Award).
- Created `prisma/seed-hr.ts` seeding: 5 Job Titles, 1 Leave Type (Annual), 1 Leave Policy, 15 Leave Balances, 3 Training Courses, 3 Training Assignments (EMP001), 5 Assets, 2 Asset Assignments (EMP001), 10 Onboarding Tasks (EMP001), 15 Payroll Profiles, 1 Payroll Run (DRAFT), 1 Employee Contract (EMP001), 3 Employee Documents (EMP001).
- Verified Excel exporter generates real .xlsx (8090 bytes test file with Summary + Daily Attendance sheets, styled headers, frozen row, auto-filter, alternating colors).
- `bun run lint`: 0 errors.
- All 14 HR routes return 307 when unauthenticated (correctly protected by middleware).
- All existing routes unchanged (public 200, protected 307).
- 52 database tables total in SQLite (verified via sqlite_master count).

Stage Summary:
- **Phase HR-1 complete.**

### Files created
- `src/lib/hr/feature-gates.ts` — HR feature gates with plan availability
- `src/lib/hr/permissions.ts` — HR permissions helper (17 permissions, role-based)
- `src/lib/excel/exporter.ts` — ExcelJS XLSX exporter with 9 functions
- `prisma/seed-hr.ts` — HR seed data
- `scripts/test-excel.ts` — Excel exporter test
- `scripts/verify-hr1.ts` — HR data verification

### Files modified
- `prisma/schema.prisma` — +19 models, +15 enums, +14 Employee fields, +15 Tenant relations, +1 Department relation
- `src/components/layout/Sidebar.tsx` — +14 HR nav items, +10 new icons

### Models added (19)
JobTitle, EmployeeContract, EmployeeDocument, LeaveType, LeavePolicy, LeaveBalance, LeaveRequest, EmployeeWarning, TrainingCourse, TrainingAssignment, EmployeeSkill, Asset, AssetAssignment, OnboardingTask, OffboardingTask, PayrollProfile, PayrollRun, PayrollRunLine, PayrollAdjustment

### Routes added
No new routes built yet — all 14 HR route paths (/hr, /hr/departments, /hr/job-titles, /hr/contracts, /hr/documents, /hr/leaves, /hr/warnings, /hr/training, /hr/assets, /hr/payroll-profiles, /hr/payroll-runs, /hr/onboarding, /hr/offboarding, /hr/reports) are protected by middleware (307 when unauthenticated) but pages will be built in HR-2 through HR-6.

### Excel exports added
Excel helper infrastructure ready (src/lib/excel/exporter.ts). API routes + page buttons will be added in HR-2.

### Permissions added (17)
VIEW_HR_DASHBOARD, MANAGE_DEPARTMENTS, MANAGE_JOB_TITLES, VIEW_EMPLOYEE_SENSITIVE_DATA, MANAGE_CONTRACTS, MANAGE_DOCUMENTS, MANAGE_LEAVE_TYPES, MANAGE_LEAVE_BALANCES, APPROVE_LEAVE, MANAGE_WARNINGS, MANAGE_TRAINING, MANAGE_ASSETS, MANAGE_ONBOARDING, MANAGE_OFFBOARDING, VIEW_PAYROLL, MANAGE_PAYROLL, EXPORT_HR_EXCEL

### Feature gates added (8)
hr_core (Trial+), hr_documents (Starter+), hr_leave (Starter+), hr_training (Growth+), hr_assets (Growth+), hr_payroll (Pro+), excel_export (Growth+), hr_excel_export (Growth+)

### Access control behavior
- Owner: all 17 HR permissions + all HR features
- HR Admin: all 17 HR permissions + all HR features (except billing platform)
- Branch Manager: VIEW_HR_DASHBOARD + APPROVE_LEAVE only; no salary/payroll access
- Employee: no HR permissions; own data only (My Profile, My Documents, My Leave, My Training, My Assets — to be built)
- Super Admin: no direct tenant HR access (audited impersonation only)

### How to test
1. `bun run db:push` — schema syncs (52 tables)
2. `bun prisma/seed-hr.ts` — seeds HR data
3. `bun scripts/test-excel.ts` — generates test .xlsx file
4. `bun scripts/verify-hr1.ts` — verifies all HR seed data
5. `bun run lint` — 0 errors
6. Visit any /hr/* route while unauthenticated → 307 redirect

### Build result
- bun run lint: 0 errors
- All 14 HR routes: 307 when unauthenticated
- All existing routes: unchanged
- Excel exporter: generates real .xlsx (8090 bytes test file)

### Known limitations
1. LeaveType uses companyId as PK (single leave type per tenant in MVP) — real system needs separate id
2. HR route pages not yet built (only middleware protection exists) — pages come in HR-2 through HR-6
3. Excel API routes not yet built (helper infrastructure ready) — routes come in HR-2
4. Payroll calculation not implemented (models + seed only) — comes in HR-5
5. No Arabic RTL export tested yet (applyRTLIfNeeded function exists but untested)

### Next phase plan (Phase HR-2)
- HR Dashboard page (/hr) with real data cards
- Departments CRUD (/hr/departments)
- Job Titles CRUD (/hr/job-titles)
- Enhanced Employee profile tabs (Overview, Attendance, Documents, Contracts, Leave, Training, Assets, Warnings, Payroll Profile, Audit)
- Employee Master Excel export
- Excel export API route (/api/reports/export?format=xlsx)
