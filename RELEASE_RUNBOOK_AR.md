...
## بوابة migrations في Vercel

لا تُنفذ migrations مع كل معاينة تلقائيًا. أصبح أمر البناء يفحص المتغير `RUN_PRISMA_MIGRATIONS`، ولا ينفذ `prisma migrate deploy` إلا عندما تكون قيمته `true`. عند التطبيق، يضبط هذا المتغير في **Production فقط** داخل Vercel؛ لا يضبط في Preview أو Development حتى لا تلامس معاينات الفروع قاعدة بيانات الإنتاج.
